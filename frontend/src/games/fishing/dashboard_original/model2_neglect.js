...
export function saveSession(patientId, sessionJson) {
  ...
  const key = 'steadyarc_neglect_sessions_' + patientId;
...
export function trainModel(patientId) {
  ...
  const rawData = sessions.map(s => [s.neglect_index, s.left_RT, s.right_RT]);
  const { standardized, means, stds } = standardize(rawData);

  const ans = kmeans(standardized, 2, { initialization: 'kmeans++' });
  
  const unstdC0_neg = ans.centroids[0].centroid[0] * stds[0] + means[0];
  const unstdC1_neg = ans.centroids[1].centroid[0] * stds[0] + means[0];
  const unstdC0_RT = (ans.centroids[0].centroid[1] * stds[1] + means[1]) + (ans.centroids[0].centroid[2] * stds[2] + means[2]);
  const unstdC1_RT = (ans.centroids[1].centroid[1] * stds[1] + means[1]) + (ans.centroids[1].centroid[2] * stds[2] + means[2]);

  const scoreC0 = -Math.abs(1.0 - unstdC0_neg) - (unstdC0_RT / 1000);
  const scoreC1 = -Math.abs(1.0 - unstdC1_neg) - (unstdC1_RT / 1000);
...
export function predictSession(newSessionData, trainResult) {
  ...
  const rawPoint = [newSessionData.neglect_index, newSessionData.left_RT, newSessionData.right_RT];
  ...
  return { score: rawScore, predictedClass: predictedCluster === trainResult.healthyClusterIndex ? 'Normal' : 'Abnormal' };
}
